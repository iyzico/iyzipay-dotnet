...
namespace Iyzipay.Model.V2.Iyzilink
{
	public class IyziLink : IyzipayResourceV2
	{
		...
		public static ResponseData<IyziLinkSave> UpdateStatus(string token, IyziLinkUpdateStatusRequest request, Options options)
		{
			var statusValue = request.Status ? "ACTIVE" : "PASSIVE";
			string uri = options.BaseUrl + V2_IYZILINK_PRODUCTS + "/" + token + $"/status/{statusValue}" + GetQueryParams(request);
			return RestHttpClientV2.Create().Patch<ResponseData<IyziLinkSave>>(uri, GetHttpHeadersWithRequestBody(request, uri, options), request);
		}
