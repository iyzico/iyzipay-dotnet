...
namespace Iyzipay.Model.V2.Subscription
{
	public class Subscription : IyzipayResourceV2
	{
		...
		public static ResponseData<CheckoutFormResult> GetCheckoutFormResult(GetCheckoutFormResultRequest request, Options options)
		{
			string uri = $"{options.BaseUrl}/v2/subscription/checkoutform/{request.Token}";
			return RestHttpClientV2.Create().Get<ResponseData<CheckoutFormResult>>(uri, GetHttpHeadersWithUrlParams(request, uri, options));
		}
